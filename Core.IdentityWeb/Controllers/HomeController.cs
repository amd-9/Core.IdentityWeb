...
using Core.IdentityWeb.Models;
using Microsoft.AspNetCore.Identity;
using Microsoft.AspNetCore.Authorization;
using System.Security.Claims;
using Microsoft.AspNetCore.Authentication;

namespace Core.IdentityWeb.Controllers
{
    public class HomeController : Controller
    {
                ...
                if (user != null && !await _userManager.IsLockedOutAsync(user))
                {
                    if (await _userManager.CheckPasswordAsync(user, model.Password))
                    {
                        ...
                        if (await _userManager.GetTwoFactorEnabledAsync(user))
                        {
                            ...
                            {
                                await HttpContext.SignInAsync(IdentityConstants.TwoFactorUserIdScheme, Store2FA(user.Id, _userManager.Options.Tokens.AuthenticatorTokenProvider));
        ...
        public async Task<IActionResult> RegisterAuthenticator()
        {
            ...
            var authenticationKey = await _userManager.GetAuthenticatorKeyAsync(user);

            if (authenticationKey == null)
            {
                await _userManager.ResetAuthenticatorKeyAsync(user);
                authenticationKey = await _userManager.GetAuthenticatorKeyAsync(user);
            }

            return View(new RegisterAuthenticatorModel { AuthenticatorKey = authenticationKey });
        }
